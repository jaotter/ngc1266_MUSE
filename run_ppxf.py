...
def ppxf_fit(cube, error_cube, vorbin_path, moments, adegree, mdegree, wave_lam, limit_doublets=True, tie_balmer=False, plot_every=0,
				plot_name=None, prev_fit_path=None, fit_gas=True, ngas_comp=1, globsearch=False, single_gascomp=False, individual_bin=None,
				test_residuals=True):
	...
	for bn in loop_list: 
		if bn >= 0: #skip nan bins with binNum == -1
			...
			if test_residuals == True:
				...
				fig.text(0.05, 0.97, f'Bin {bn}. Number of Spectra in Bin: {n_spec}', fontsize = 12)
				fig.text(0.05, 0.93, 'A/rN of Halpha: '+str(save_dict['Halpha_(1)_ANR'][-1]))
				fig.text(0.45, 0.93, f'Chi-Squared/DOF: {int(np.round(pp.chi2,-1))}')
				...
				full_plot_dir = f'{plot_dir}{plot_name}'
				plot_fl = f'{full_plot_dir}/ppxf_residuals_bin{bn}_{ngas_comp}comp.png'
...
def ppxf_fit_individual(cube, error_cube, vorbin_path, moments, adegree, mdegree, wave_lam, limit_doublets=True, tie_balmer=False, plot_every=0,
				plot_name=None, prev_fit_path=None, fit_gas=True, ngas_comp=1, globsearch=False, single_gascomp=False, individual_bin=None,
				test_residuals=True):
	#cube - unbinned data cube
	#error_cube - unbinned error cube
...
def ppxf_iterative_fit(cube, error_cube, vorbin_path, moments, adegree, mdegree, wave_lam, limit_doublets=True, tie_balmer=False, plot_every=0,
	...
	for bn in loop_list: 
		if bn >= 0: #skip nan bins with binNum == -1
			...
			if Halpha_anr >= 10:
				...
				full_plot_dir = f'{plot_dir}{plot_name}'
				plot_fl = f'{full_plot_dir}/ppxf_residuals_bin{bn}_{ngas_comp_start}comp.png'
			...
			if plot_every > 0 and bn % plot_every == 0:
				...
				full_plot_dir = f'{plot_dir}{plot_name}'
				plot_fl = f'{full_plot_dir}/ppxf_gasfit_bin{bn}_{ncomp_final}ncomp.png'
